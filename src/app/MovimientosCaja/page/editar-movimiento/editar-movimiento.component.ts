...
export class EditarMovimientoComponent implements OnInit, OnDestroy {
  titulo = 'Editar Movimiento de Caja';
  ...
  private checkEditMode(): void {
    this.route.params.pipe(
      ...
      if (id) {
        ...
        this.snackBar.open('ID de movimiento no válido', 'Cerrar', {
          duration: 3000,
  ...
  private loadMovimiento(id: string): void {
    ...
    // Usar un método específico que maneje el problema del enum
    this.movimientoCajaService.getMovimientoForEdit(id).pipe(
      takeUntil(this.destroy$),
      catchError(error => {
  ...
  guardar(): void {
    ...
    try {
      ...
      this.loading = true;
      this.movimientoCajaService.update(this.movimientoId!, movimientoData).pipe(
        takeUntil(this.destroy$),
        catchError((error) => {
          console.error('Error al actualizar movimiento:', error);
          this.loading = false;
          
          let mensajeError = 'Error al actualizar el movimiento';
          ...
          if (error.message && error.message.includes('llave duplicada')) {
            ...
            mensajeError = 'Error al actualizar el movimiento: ' + error.message;
          }
        ...
        if (result) {
          this.loading = false;
          this.snackBar.open('Movimiento de caja actualizado exitosamente', 'Cerrar', {
            duration: 3000,
