...
import { CajeroReporteService } from '../../components/cajero.reporte.service';
...
@Component({
  selector: 'app-reporte-cajero',
  templateUrl: './reporte-cajero.component.html',
  styleUrls: ['./reporte-cajero.component.scss']
})
export class ReporteCajeroComponent implements OnInit, OnDestroy {
  reporteForm: FormGroup;
  isLoading = false;
  private destroy$ = new Subject<void>();

  constructor(
    private cajeroReporteService: CajeroReporteService,
  ...
  descargarReporte(): void {
    ...
    this.cajeroReporteService.descargar(
      formValue.nombre, 
      fechaInicioStr, 
      fechaFinStr
    ).pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (res) => {
        this.showPdfInBrowser(res, 'reporte-cajeros.pdf');
